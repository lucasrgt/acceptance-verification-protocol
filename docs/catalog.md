...
- **STATIC** — best caught before runtime, by static analysis. These belong in the
  host project's linter/doctor (in this codebase's case, the Skies `SKY*`/`SKYFE*`
...
Already enforced statically by the host project's doctor (`SKYFE010`). Listed for
...
`SKYFE011` (locale parity) + `SKYFE014` (no hardcoded copy). Static-doctor territory.
...
| **STATIC — host doctor** | contract-mints-no-routes, state-completeness, i18n-honesty, money-is-typed, money-formatted-once | the host project's linter (Skies `SKY*`/`SKYFE*`) |
...
- **~15% are best left static** — already absorbed by the host project's doctor as
  `SKY*`/`SKYFE*` rules. Several escapes here *became* doctor rules after the fact
  (e.g. the redirect-loop fix shipped `SKYFE015`, the navigation-cast ban is
  `SKYFE030`). That organic migration — escape → static rule — is the convergence
